 ...
 * @param 
 */
class TouchControl extends _dreamengine.GameObject {
  constructor(params) {
    const {
      ...
      onStickMoved,
      onStickReleased
    } = params; // Check that mandatory parameters are correct 
    ...
    if (!(onStickReleased instanceof Function)) console.error('optional parameter "onStickReleased" must be a function'); // Set the background renderer and instantiate the GameObject

    params.renderers = [backgroundSpriteRenderer];
    super(params); // Create the Stick and attach it to the TouchControl

    this.stick = new _Stick.default({
      renderers: [stickSpriteRenderer]
    });
    this.add(this.stick); // Bind the onStickMoved and onStickReleased (if exists) callback
